...
description: Playwright-MCP を使用して、ローカルサーバーで起動している Marp スライドを視覚的に検証し、レイアウト問題を自動修正するスキル。「スライドをレビュー」「Marp スライドの視覚的チェック」「スライドのレイアウトを確認」「プレゼン資料の見た目を検証」などのリクエストでトリガーします。テキストの切れ目、重なり、配置問題、コントラスト不足、はみ出し、余白不足などの視覚的問題を検出・修正します。
---

# Marp スライドレビュアー

このスキルは、Playwright-MCP を使用して Marp スライドの視覚的な品質を検証し、問題を自動修正します。
...
1. 指定 URL にアクセス（`mcp_playwright_browser_navigate`）
2. スナップショット取得（`mcp_playwright_browser_snapshot`）
...
2. スクリーンショット撮影（`mcp_playwright_browser_take_screenshot`）
3. 画像を分析し、視覚的問題を確認
...
1. 元の Markdown ファイルを読み込む（`read_file`）
2. 問題スライドのセクションを特定（`---` で区切られた箇所）
3. 修正を適用（`replace_string_in_file`）
